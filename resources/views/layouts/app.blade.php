...
<header class="p-5 border-b bg-white shadow flex justify-between items-center">
    ...
    @auth()
        <nav class="flex gap-2 items-center">
            <a href="{{route('post.create')}}" class="flex gap-2 items-center bg-white border p-2 text-gray-600 rounded text-sm uppercase font-bold cursor-pointer" href="{{route('post.create')}}">
            ...
            <form action="_{{ route('logout') }} " method="post">
                @csrf
